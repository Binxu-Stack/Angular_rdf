...
void ComputeAngularRDF::compute_array()
{
  ...
  double cos_theta_hi = cos(angle_lo);
  double cos_theta_lo = cos(angle_hi);
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if ( cos_theta < cos_theta_lo || cos_theta > cos_theta_hi) continue;
  ...
  if (domain->dimension == 3) {
    ...
    constant = 2.0 * MY_PI * dtheta / (3.0*domain->xprd*domain->yprd*domain->zprd);
