...
ComputeAngularRDF::ComputeAngularRDF(LAMMPS *lmp, int narg, char **arg) :
  ...
  duplicates(NULL)
{
  ...
  for (iarg = 4; iarg < narg; iarg++)
    if (strcmp(arg[iarg],"cutoff") == 0 || strcmp(arg[iarg],"angle") == 0) break;

  int nargpair = iarg - 4;
  // default angle region
  angle_lo = angle_hi = 0.0;
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"cutoff") == 0) {
      ...
      angle_lo = force->numeric(FLERR, arg[iarg+1]);
      angle_hi = force->numeric(FLERR, arg[iarg+2]);
      iarg += 3;
      //fprintf(screen, "\nLow angle: %f, High angle: %f\n", angle_lo, angle_hi);
  ...
  memory->create(array,nbin,1+3*npairs,"rdf:array");
...
ComputeAngularRDF::~ComputeAngularRDF()
{
...
void ComputeAngularRDF::init_list(int id, NeighList *ptr)
{
  list = ptr;
}
...
void ComputeAngularRDF::init_norm()
{
  int i,j,m;
...
void ComputeAngularRDF::compute_array()
{
  ...
  double dtheta = angle_hi - angle_lo;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      // atom j in the specific angle region
      //double rxy = sqrt(delx*delx+dely*dely);
      itheta = atan2(dely,delx);
      jtheta = atan2(-dely,-delx);
      if (dely < 0) itheta = MY_PI*2.0 + itheta;
      if (-dely < 0) jtheta = MY_PI*2.0 + jtheta;
      //if (theta < angle_lo || theta > angle_hi) continue;

      if (ipair) {
	if (itheta >= angle_lo && itheta < angle_hi) { 
          for (ihisto = 0; ihisto < ipair; ihisto++)
            //hist[rdfpair[ihisto][itype][jtype]][ibin] += 1.0;
            //hist[rdfpair[ihisto][itype][jtype]][ibin] += 1.0/sin(theta);
            hist[rdfpair[ihisto][itype][jtype]][ibin] += 1.0;
	}
      }
      if (newton_pair || j < nlocal) {
        if (jpair)
	  if (jtheta >= angle_lo && jtheta < angle_hi) {
            for (ihisto = 0; ihisto < jpair; ihisto++)
              //hist[rdfpair[ihisto][jtype][itype]][ibin] += 1.0;
              //hist[rdfpair[ihisto][jtype][itype]][ibin] += 1.0/sin(theta);
	      //if (jtheta < angle_lo || jtheta > angle_hi) continue;
              hist[rdfpair[ihisto][jtype][itype]][ibin] += 1.0;
	  }
      }
    }
  ...
  // s2
  //double* s2 = new double[npairs];
  //for (m = 0; m < npairs; ++m){
  //  s2[m] = 0.;
  //}
  // number density
  double rho = atom->natoms / (domain->xprd*domain->yprd*domain->zprd);

  if (domain->dimension == 3) {
    //constant = 4.0*MY_PI / (3.0*domain->xprd*domain->yprd*domain->zprd);
    constant = 2.0 * dtheta / (3.0*domain->xprd*domain->yprd*domain->zprd);
	...
	if (gr > 0.0) {
	  array[ibin][3+3*m] = ((ibin == 0) ? 0.5*-rho*(gr * log(gr) - (gr - 1)) * array[ibin][0]*array[ibin][0] * delr :  array[ibin-1][3+3*m] -
	   rho*(gr * log(gr) - (gr - 1)) * array[ibin][0]*array[ibin][0] * delr );
	}else{
	  array[ibin][3+3*m] = ((ibin == 0) ? 0.5*-rho*array[ibin][0]*array[ibin][0]*delr : array[ibin-1][3+3*m] -
	      rho*array[ibin][0]*array[ibin][0] * delr);
	}
	//if (gr > 0.0){
	//  s2[m] += -rho*(gr * log(gr) - (gr - 1)) * array[ibin][0]*array[ibin][0] * delr;
	//}
      }
    }
    ...
    constant = dtheta / (domain->xprd*domain->yprd);
  ...
  //if (screen && comm->me == 0) {
  //  fprintf(screen, "\n s2 info (angle, s2)\n! %10f ", (angle_hi+angle_lo)/2);
  //  for (m = 0; m < npairs; ++m){
  //    fprintf(screen, "%16f ",s2[m]);
  //  }
  //  fprintf(screen,"\n");
  //}
